...
class GCNModelVAE(Model):
    ...
    def __init__(self, placeholders, num_features, num_nodes, features_nonzero, **kwargs):
        ...
        self.adj_pre = placeholders['adj_pre']
        self.dropout = placeholders['dropout']
        self.adj_label = placeholders['adj_post_orig']
        self.weight_norm = 0
        self.build()
    ...
    def get_z(self, random):
        ...
        z = self.z_mean + tf.compat.v1.random_normal([self.n_samples, FLAGS.hidden2]) * tf.exp(self.z_log_std)
...
class GCNModelFeedback(GCNModelVAE):
    ...
    def decoder(self, z):
        ...
        recon_1 = tf.compat.v1.nn.l2_normalize(z, dim = 1)
    ...
    def sample(self):
        z = tf.compat.v1.random_normal([self.n_samples, FLAGS.hidden2])
...
class GCNModelSiemens(GCNModelVAE):
    ...
    def make_decoder(self):
        ...
        self.l3 = Dense(input_dim=2*FLAGS.hidden2,      #2*FLAGS-hidden2
                                              output_dim=FLAGS.hidden3,
                                              act=tf.nn.elu,
                                              dropout=self.dropout,
                                              bias=True,
    ...
    def decoder(self, z):
        ...
        update = self.l1(z) + self.l0(tf.compat.v1.sparse_tensor_to_dense(self.inputs))
        ...
        #a=tf.nn.l2_normalize(update,1)#cosine similarity
        #prod=tf.matmul(a,a,adjoint_b=True)
        #sim=1-prod
...
class myModel(Model):
    '''Model for generating graphs from multiple training graphs'''
    def __init__(self, placeholders, num_features, num_nodes, features_nonzero, keep_features_for_rois, **kwargs):
        ...
        self.adj_label = placeholders['adj_post_orig']
        self.weight_norm = 0
        self.keep=keep_features_for_rois
        self.build()
    ...
    def encoder(self, inputs):
        ...
        hidden1 = GraphConvolutionSparse(input_dim=self.input_dim,
                                              ...
                                              features_nonzero=self.features_nonzero,
                                              keep_features_for_rois=self.keep,
                                              act=tf.nn.relu,
                                              dropout=0.,
                                              logging=self.logging)(inputs)
    ...
    def decoder(self, z):
        ...
        reconstructions = tf.reshape(update, [-1])
        #self.full_recon = tf.nn.sigmoid(reconstructions)
        return reconstructions
    ...
    def _build(self):
        ...
        self.embedding=z_noiseless  
        self.reconstructions = self.decoder(z)
        self.reconstructions_noiseless = tf.nn.sigmoid(self.decoder(z_noiseless))
        self.reconstructions_noiseless = tf.reshape(self.reconstructions_noiseless, [self.n_samples, self.n_samples])
        self.reconstructions_noiseless_full = self.decoder(z_noiseless)
        self.reconstructions_noiseless_full = tf.reshape(self.reconstructions_noiseless_full, [self.n_samples, self.n_samples])
