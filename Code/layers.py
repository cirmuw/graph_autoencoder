...
def dropout_sparse(x, keep_prob, num_nonzero_elems, keep_features_for_rois,num_features,num_nodes):
    ...
    if keep_features_for_rois is not None:
        indices=tf.constant([[keep_features_for_rois[i]] for i in range(len(keep_features_for_rois))])
        updates=tf.reshape(dropout_mask,shape=(len(keep_features_for_rois),num_features))
        shape=tf.constant([num_nodes,num_features])
        dropout_mask=tf.scatter_nd(indices,updates,shape)
        dropout_mask=tf.reshape(dropout_mask,shape=(-1,))
...
class GraphConvolutionSparse(Layer):
    ...
    def __init__(self, input_dim, output_dim, adj,num_nodes, features_nonzero,keep_features_for_rois=None, dropout=0., act=tf.nn.relu, **kwargs):
    ...
    def _call(self, inputs):
        x = inputs
        x = dropout_sparse(x, 1-self.dropout, self.features_nonzero, self.keep,self.num_features,self.num_nodes)
        x = tf.compat.v1.sparse_tensor_dense_matmul(x, self.vars['weights'])
        x = tf.compat.v1.sparse_tensor_dense_matmul(self.adj, x)
        outputs = self.act(x)
        return outputs
